---
title: "接收器"
merged_source: "current_waveda_agent_kb"
source_relative_path: "EM_Project/Observer/Observer.html"
content_kind: "markdown"
merged_at: "2026-07-07"
---

# 接收器

- 原始相对路径: `EM_Project/Observer/Observer.html`
- 知识模块: `EM设计与结果`

## 正文抽取
## 接收器

- 创建点接收器 - 创建线接收器 - 创建新组 - 从文件导入接收器信息 - 导出接收器信息到文件 - 其他功能

右键点击工程树上的接收器，可创建接收器，仿真完成后可查看接收器上的场信息。

> 图示要点：`Observer_1.png` 展示“接收器”相关的操作界面或示例，后续审查通过后再补图。

### 创建点接收器

点击创建点接收器，输入坐标，可建立点接收器，具体操作见创建点接收器文档。

> 图示要点：`Observer_2.png` 展示“创建点接收器”相关的操作界面或示例，后续审查通过后再补图。

### 创建线接收器

点击创建线接收器，输入坐标，可建立线接收器，具体操作见创建线接收器文档。

> 图示要点：`Observer_3.png` 展示“创建线接收器”相关的操作界面或示例，后续审查通过后再补图。

### 创建新组

可通过创建分组来对点接收器进行分组。

### 从文件导入接收器信息

点击从文件导入接收器信息，可导入接收器坐标信息，仅支持txt文档格式。

### 导出接收器信息到文件

点击导出接收器信息到文件，可导出接收器坐标信息，支持txt、csv、xls文档格式。

### 其他功能

除上述功能外，接收器选项还支持删除所有接收器、接收器的复制粘贴、展开或者折叠接收器分组的所有节点信息。

## 页内/相关链接

- - 创建点接收器: `#创建点接收器`
- - 创建点接收器 - 创建线接收器: `#创建线接收器`
- - 创建点接收器 - 创建线接收器 - 创建新组: `#创建新组`
- - 创建点接收器 - 创建线接收器 - 创建新组 - 从文件导入接收器信息: `#从文件导入接收器信息`
- - 创建点接收器 - 创建线接收器 - 创建新组 - 从文件导入接收器信息 - 导出接收器信息到文件: `#导出接收器信息到文件`
- - 创建点接收器 - 创建线接收器 - 创建新组 - 从文件导入接收器信息 - 导出接收器信息到文件 - 其他功能: `#其他功能`
- 点击创建点接收器，输入坐标，可建立点接收器，具体操作见创建点接收器: `./Create_Point_Observer.html`
- 点击创建线接收器，输入坐标，可建立线接收器，具体操作见创建线接收器: `./Create_Line_Observer.html`


## 待补图片清单
以下图片暂不插入正文，路径为 WavEDA 帮助文档内部相对路径；运行时 agent 会按项目内帮助图片和用户本机 WavEDA 帮助目录进行查找。
| 图片名称 | WavEDA 帮助相对路径 | 建议保留原因 |
| --- | --- | --- |
| Observer_1.png | `EM_Project/Observer/images/Observer_1.png` | 展示“接收器”相关界面或示例，后续审查时判断是否需要作为辅助配图。 |
| Observer_2.png | `EM_Project/Observer/images/Observer_2.png` | 展示“创建点接收器”步骤的菜单或参数界面，便于新人按界面完成操作。 |
| Observer_3.png | `EM_Project/Observer/images/Observer_3.png` | 展示“创建线接收器”步骤的菜单或参数界面，便于新人按界面完成操作。 |
