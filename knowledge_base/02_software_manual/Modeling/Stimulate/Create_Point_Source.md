...
source_relative_path: "Modeling/Stimulate/Create_Point_Source.html"
...
> 图示要点：`Create_Point_Source_1.png` 展示“点激励源的应用场景：”相关的操作界面或示例，后续审查通过后再补图。
...
> 图示要点：`Create_Point_Source_2.png` 展示“形式”相关的操作界面或示例，后续审查通过后再补图。
...
> 图示要点：`Create_Point_Source_6.png` 展示“阵列排序”相关的操作界面或示例，后续审查通过后再补图。

效果图如下，方向框后数值代表着xyz方向，为0则无位移，大小表示该方向的位移。数量框后数值代表该方向的阵列量。整体的阵列排序个数为三个方向数量的乘积。

> 图示要点：`Create_Point_Source_5.png` 展示“阵列排序”相关的操作界面或示例，后续审查通过后再补图。
...
| 图片名称 | 当前资源路径 | 建议保留原因 |
| --- | --- | --- |
| Create_Point_Source_1.png | `wavEDA_docs/helpHtml/helpHtml/Modeling/Stimulate/images//Create_Point_Source_1.png` | 展示“点激励源的应用场景：”相关界面或示例，后续审查时判断是否需要作为辅助配图。 |
| Create_Point_Source_2.png | `wavEDA_docs/helpHtml/helpHtml/Modeling/Stimulate/images/Create_Point_Source_2.png` | 展示“形式”相关界面或示例，后续审查时判断是否需要作为辅助配图。 |
| Create_Point_Source_6.png | `wavEDA_docs/helpHtml/helpHtml/Modeling/Stimulate/images/Create_Point_Source_6.png` | 展示“阵列排序”相关界面或示例，后续审查时判断是否需要作为辅助配图。 |
| Create_Point_Source_5.png | `wavEDA_docs/helpHtml/helpHtml/Modeling/Stimulate/images/Create_Point_Source_5.png` | 展示“阵列排序”相关界面或示例，后续审查时判断是否需要作为辅助配图。 |
