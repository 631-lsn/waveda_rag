...
> **167 个 Markdown 源文件；其中 165 个进入常规语义索引，另外 2 个原始错误表保留在仓库中供维护和精确查询使用**
...
> **当前全量索引：263 个文档、1732 个 chunk、约 541K 字符、1732×384 向量矩阵**

```
data/index/
├── chunks.json        约 2.1 MB    1732 个知识块（含完整文本 + 元数据）
├── vectors.npy        约 2.5 MB    1732×384 float32 哈希向量矩阵
...
- 当前 1732 个向量继续使用 NumPy 矩阵检索。达到数万分块并出现实测瓶颈后，再评估 FAISS 或 LanceDB，避免提前增加 EXE 依赖与体积。
